...
# Substitute this with the functions I will create to substitute existing bash code in run file
...
# use this when I bidsify this gear
...
# get additional params from config.json
...
subject_id = conf['config']['subject_id']
inDir        = path.join(flywheelBase, 'input')
outDir       = path.join(flywheelBase, 'output')
...
# fsDir    = path.join(inDir, f'{subject_id}')
fsDir    = path.join(outDir, f'{subject_id}')
if path.isdir(fsDir):
...
if not path.isfile(path.join(fsDir, 'surf', 'lh.benson14_varea.mgz')):
    try:
